...
const readAllDbgFiles = () => {
    const dbgFiles = fs.readdirSync(`${__dirname}/../artifacts/contracts`);
    const buildFilePaths = [];
    dbgFiles.forEach((file) => {
        const files = fs.readdirSync(`${__dirname}/../artifacts/contracts/${file}`);
        files.forEach((f) => {
            if (f.includes('.dbg.json')) {
                //read the content of the file
                //remove .dbg.json from file name string
                const fileName = f.replace('.dbg.json', '');
                const content = JSON.parse(fs.readFileSync(`${__dirname}/../artifacts/contracts/${file}/${f}`, 'utf8'));
...
const main = async () => {
    ...
    buildFilePaths.forEach((file) => {
        ...
        const content = JSON.parse(fs.readFileSync(`${__dirname}/../artifacts/build-info/${file.buildInfoId}.json`, 'utf8'));
